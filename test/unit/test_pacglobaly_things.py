...
def test_valid_pacglobal_address():
    from pacgloballib import is_valid_pacglobal_address

    main = valid_pacglobal_address()
    test = valid_pacglobal_address('testnet')

    assert is_valid_pacglobal_address(main) is True
    assert is_valid_pacglobal_address(main, 'mainnet') is True
    assert is_valid_pacglobal_address(main, 'testnet') is False

    assert is_valid_pacglobal_address(test) is False
    assert is_valid_pacglobal_address(test, 'mainnet') is False
    assert is_valid_pacglobal_address(test, 'testnet') is True


def test_invalid_pacglobal_address():
    from pacgloballib import is_valid_pacglobal_address

    main = invalid_pacglobal_address()
    test = invalid_pacglobal_address('testnet')

    assert is_valid_pacglobal_address(main) is False
    assert is_valid_pacglobal_address(main, 'mainnet') is False
    assert is_valid_pacglobal_address(main, 'testnet') is False

    assert is_valid_pacglobal_address(test) is False
    assert is_valid_pacglobal_address(test, 'mainnet') is False
    assert is_valid_pacglobal_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from pacgloballib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % pacgloballib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(pacgloballib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(pacgloballib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(pacgloballib.blocks_to_seconds(16616)) == 2612035
