...
def is_valid_pacglobal_address(address, network='mainnet'):
    ...
    pacglobal_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid PACGlobal address
        return False

    if (address_version != pacglobal_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('pacglobal.conf')
