...
def has_pacglobal_conf():
    import config
    import io

    valid_pacglobal_conf = False

    # ensure pacglobal_conf exists & readable
    #
    # if not, print a message stating that PACGlobal Core must be installed and
    # configured, including JSONRPC access in pacglobal.conf
    try:
        f = io.open(config.pacglobal_conf)
        valid_pacglobal_conf = True
    except IOError as e:
        print(e)

    return valid_pacglobal_conf
...
def main():
    ...
    if not has_pacglobal_conf():
        print("PACGlobal must be installed and configured, including JSONRPC access in pacglobal.conf")
        sys.exit(1)
