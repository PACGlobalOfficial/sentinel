...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self,pacglobald):
        golist = pacglobald.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_pacglobald(pacglobald, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_pacglobald(self,pacglobald, rec):
        import decimal
        import pacgloballib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from pacglobald...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from pacglobald, with every run
        ...
        # get/create, then sync payment amounts, etc. from pacglobald -pacglobald is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(pacglobald)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from pacglobald! %s" % e)
            govobj.vote_delete(pacglobald)
            return (govobj, None)
    ...
    def vote_delete(self,pacglobald):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(pacglobald, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self,pacglobald, signal, outcome):
        import pacgloballib
        ...
        output = pacglobald.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = pacgloballib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(pacglobald, signal)

    def sync_network_vote(self,pacglobald, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = pacglobald.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = PACGlobalD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 PACGlobal addr, non-multisig
            if not pacgloballib.is_valid_pacglobal_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid PACGlobal address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # PACGlobal Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (pacgloballib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = PACGlobalD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not pacgloballib.is_valid_pacglobal_address(addr, config.network):
    ...
    def hash(self):
        import pacgloballib
        return pacgloballib.hashit(self.serialise())
