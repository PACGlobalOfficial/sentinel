...
class Masternode():
    ...
    def parse_mn_string(self, mn_full_out):
        ...
        try:
            (status, address, lastseen, activeseconds, lastpaid, ip_port) = mn_full_out.split()
        except:
            (status, protocol, address, lastseen, activeseconds, lastpaid, lastpaidblock, ip_port) = mn_full_out.split()
